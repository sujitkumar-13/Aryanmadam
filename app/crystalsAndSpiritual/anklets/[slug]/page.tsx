...
export default function AnkletCategoryPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/anklets?category=${slug}`);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {products.map((p) => {
                ...
                return (
                  ...
                  >
                    {/* IMAGE - Clickable */}
                    <Link href={`/crystalsAndSpiritual/anklets/product/${p.id}`} className="block">
                    ...
                    {/* TITLE - Clickable */}
                    <Link href={`/crystalsAndSpiritual/anklets/product/${p.id}`}>
